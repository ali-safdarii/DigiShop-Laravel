<div class="shadow-3 card mt-5 p-3">
    ...
    <div class="d-flex justify-content-between m-3 p-3 border-bottom">
        ...
        <a href="{{route('market.payments.create')}}">
            <button class="btn btn-primary">Create
                <i class="fas ms-1 fa-add"></i>
            </button>
        </a>

        <h5 wire:loading.remove>Payments ({{\App\Models\Admin\Payment\Payment::count()}})</h5>
        ...
        <table class="table-responsive table">
            ...
            @forelse($payments as $payment)
                <tr>
                    ...
                    <th scope="row">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" value="{{ $payment->id }}"
                                   id="checkbox-{{ $payment->id }}" wire:model="selectedItems">
                    ...
                    <td>
                        {{\App\Utili\Helper::limitStr($payment->user->FullName)}}
                    </td>
                    <td>
                    ...
                    <td>
                      ...
                      {{--  @if($payment->status == 'Completed')
                            <img class="" src="{{ asset('admin/images/icons/correct.png') }}" alt="{{ $payment->status }}" width="20" height="20"/>
                        @elseif($payment->status == 'Cancelled')
                            <img class="" src="{{ asset('admin/images/icons/remove.png') }}" alt="{{ $payment->status }}" width="20" height="20"/>
                        @endif--}}


                    </td>
                    <td>
                        <a class="btn btn-primary" type="submit" href="{{route('market.payments.show',$payment)}}"
                           role="button">
                            show
