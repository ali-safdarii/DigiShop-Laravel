...
@section('content')
    ...
    <div class="main-content ">
        ...
        <x-custom-component.delete-modal :message="'Posts?'"
                                         :route=" route('content.post.destroy', ['post' => $post->id]) "
        />

        <form id="post-category-form" class="needs-validation"
              action="{{route('content.post.update',$post->id)}}" method="post"
            ...
            <div class="d-flex flex-wrap justify-content-center align-items-center card
                ...
                <div class="col-sm-12 col-md-6 col-lg-6 p-3">
                    ...
                    <div class="form-content ">
                        ...
                        <div class="form mt-3">
                            ...
                            <select class="form-select" name="category_id" aria-label="category_id" id="category_id"
                                ...
                                @foreach($categories as $cat)
                                    <option value="{{$cat->id}}" {{ $post->postCategory->id == $cat->id ? 'selected' : '' }} >{{$cat->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form mt-3">
                            <label class="form-label" for="is_comment">Block comment</label>
                            <select class="form-select" name="is_comment" aria-label="is_comment" id="is_comment"
                                    required>
                                <option value="1" {{ $post->is_comment == 1 ? 'selected' : '' }}>Enabled</option>
                                <option value="0" {{ $post->is_comment == 0 ? 'selected' : '' }}>Disabled</option>
                            </select>
                        </div>
